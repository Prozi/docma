...
const path = {
    ...
    parentOfBase(p, base) {
        ...
        return p.slice(-base.length) === base ? p.slice(0, -base.length) : p;
    },
...
const glob = {
    ...
    match(patterns, options = {}) {
        ...
        return Bluebird.each(properPatterns, (p) => {
...
// https://onedayitwillmake.com/blog/2013/03/compiling-less-from-a-node-js-script/
// https://lesscss.org/#using-less-command-line-usage
...
const less = {
    ...
    compileFile(filePath, options) {
        return fs.readFile(filePath, "utf8").then((strLess) => {
...
utils = {
    ...
    notate(obj, notation) {
        ...
        const props = !Array.isArray(notation) ? notation.split(".") : notation;
        const prop = props[0];
        if (!prop) return;
        const o = obj[prop];
        if (props.length > 1) {
