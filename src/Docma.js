...
class Docma {
    ...
    _init(config) {
        ...
        if (this.config.app.base) {
            this.config.app.base = utils.path.ensureEndSlash(
                this.config.app.base,
            );
        }
    ...
    _addRoute(name, sourceType) {
        ...
        if (RE_INVALID_GROUP_NAME.test(name)) {
            throw new Error(
                'Route/path name "api" is a reserved word. Any ungrouped JS files will already be merged under "api" default route.',
            );
        }

        if (RE_INVALID_GROUP_CHARS.test(name)) {
            throw new Error(
                `'${name}' has some invalid characters for valid Docma path name.`,
            );
        }
        ...
        if (!queryRouting && !isApache) {
            ...
            if (!utils.path.hasValidName(name, isWin)) {
                throw new Error(
                    `'${name}' is not a valid path name for the configured server type '${this.config.app.server}'.`,
                );
            }
        }
        ...
        if (_.find(this.routes, { type, name })) {
            throw new Error(
                `Cannot have duplicate route name '${name}' with the same route type '${type}'.`,
            );
        }
        ...
        this.routes.push({
            ...
            // content directory.
            contentPath: !isApiRoute ? `content/${name}.html` : null,
            sourceType,
        });
    }
            ...
            fs
                ...
                .then(() => {
                    return Promise.each(Paths.DOCMA_COMPS, (file) => {
                        this.$debug.data(
                            "Compiling:",
                            path.relative(Paths.DIR_WEB, file),
                        );
                        return utils.js
                            .readFile(file, this.$debug.noMinify, true)
                            .then((code) => {
                    ...
                    return (
                        Promise.resolve()
                            ...
                            .then(() => {
                                ...
                                docmaWeb.push(`${code}\n`);
                                docmaWeb.push(
                                    `DocmaWeb.version = "${pkg.version}";`,
                                );
                                docmaWeb.push("return DocmaWeb;\n})();");
                                docmaWeb.push(
                                    `var docma = Object.freeze(new DocmaWeb(${JSON.stringify(
                                        docmaData,
                                    )}));\n`,
                                );
                            })
                ...
                .then(() => {
                    ...
                    this.stats.scriptSizeGZipped = utils.getContentSize(
                        script,
                        true,
                    );
    ...
    _processFiles() {
        ...
        function pick(memo, src, name) {
            ...
            const patt =
                typeof $this.config.jsdoc.includePattern === "string"
            ...
            return utils.glob.match(srcPath).then((srcFiles) => {
                _.each(srcFiles, (filePath) => {
                    ...
                    if (pinfo.forced && pinfo.parserType) {
                        $this.$debug.warn(
                            ` » ${pinfo.parserType} parser will be forced on: "${filePath}"`,
                        );
                    }
                    ...
                    if (jsdocInclude || pinfo.parserType === "JSDoc") {
                        ...
                        const grp = (memo.js[jsGroupName] =
                            memo.js[jsGroupName] || []); // eslint-disable-line
                        ...
                        const mdName = (
                            name || utils.path.basename(filePath)
                        ).toLowerCase();
        ...
        return Promise.reduce(
            ...
            (memo, src) => {
                ...
                if (_.isPlainObject(src)) {
                    return Promise.each(_.keys(src), (key) =>
                        pick(memo, src[key], key),
                    ).thenReturn(memo);
                }
                return memo;
            },
    ...
    _parseJS(jsFiles, filePrefix) {
        ...
        if (this.$debug.jsdocOutput) {
            let filename =
                filePrefix && filePrefix !== DEFAULT_JS_GROUP
                    ? filePrefix + "."
                    : ""; // eslint-disable-line
            filename = filename.replace(/\//g, "_") + JSDOC_OUT_SUFFIX; // some route names might have / (deep path name)
            jdxOpts.output = {
    ...
    _copyAssets() {
        ...
        return Promise.each(targetDirs, (dir) => {
            ...
            return utils.glob.match(copyList).then((files) => {
        ...
        return (
            Promise.resolve()
                ...
                .then((conf) => {
                    ...
                    this.$debug.log(
                        "Using Template:",
                        this.templateBuilder.template.name,
                        `(version: ${this.templateBuilder.template.version})`,
                    );
                    if (
                        semver.lt(
                            this.templateBuilder.template.version,
                            "1.0.0",
                        )
                    ) {
                        this.$debug.warn(
                            "Note that this template may not be production-ready since it has a version less than 1.0.0",
                        );
                    }
                ...
                .then(() => {
                    ...
                    const caseSensitiveRouting =
                        this.config.app.routing.caseSensitive;
                    return Promise.reduce(
                        groupNames,
                        (memo, name) => {
                            ...
                            this.$debug.data(
                                `Parsing (${name}): ${group.length} files`,
                            );
                            return this._parseJS(group, name).then((data) => {
                                this._addRoute(name, "js"); // e.g. name=web —> api:web
                                memo[name] = {
                                    documentation: data,
                                    symbols: jsdocx.utils.getSymbolNames(
                                        data,
                                        this.config.jsdoc.sort,
                                    ),
                                };
                                return memo;
                            });
                ...
                .then(() => {
                    ...
                    this.$debug.data(
                        "Script Size: ",
                        this.stats.scriptSize,
                        "KB",
                        this.$debug.noMinify
                            ? "(unminifed, fully documented),"
                            : "(minified),",
                        this.stats.scriptSizeGZipped,
                        "KB",
                        "(gzipped)",
                    );
                    this.$debug.data(
                        ...
                        "(" +
                            this.stats.apiRouteCount +
                            " API, " +
                            this.stats.contentRouteCount +
                            " content)",
                    );
                    if (this.$debug.buildLogs) {
                    ...
                    if (this.$debug.noMinify || this.$debug.webLogs) {
                        ...
                        if (this.$debug.noMinify)
                            msg.push("minification is turned off");
                        if (this.$debug.webLogs)
                            msg.push("web-logs is enabled");
                        msg = msg.join(" and ");
