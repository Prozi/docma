...
function _addMetasToDoc($, metaList) {
    ...
    if (existingMetas.length > 0) {
        ...
        lastMeta =
            HtmlParser.DOM.N_TAB + HtmlParser.DOM.getMetaElem(metaList[0]);
        lastMeta = $head.prepend(lastMeta).find("meta");
        metaList.shift(); // remove first
    }
    metaList.forEach((metaInfo) => {
        const meta =
            HtmlParser.DOM.N_TAB + HtmlParser.DOM.getMetaElem(metaInfo);
...
class TemplateBuilder {
    ...
    _getModuleInfo(nameOrPath, safe = true) {
        const fn = safe ? utils.safeRequire(nameOrPath) : require(nameOrPath);
    ...
    _initTemplateModule(nameOrPath) {
        let moduleInfo;
        if (
            ["default", "zebra", "docma-template-zebra"].indexOf(nameOrPath) >=
            0
        ) {
            ...
            if (!moduleInfo) {
                ...
                moduleInfo = this._getModuleInfo(
                    path.join(__dirname, "..", "templates", "zebra"),
                    false,
                );
            }
        ...
        if (!moduleInfo) {
            throw new Error(
                `Could not load Docma template module: ${nameOrPath}`,
            );
        }
    ...
    checkVersion() {
        return semver.satisfies(
            docmaPkg.version,
            this.template.supportedDocmaVersion,
        );
    }
    ...
    compileDocmaStyles() {
        const compileOpts = {
            filename: path.resolve(__dirname, "web", Paths.DOCMA_LESS), // less file name not the path
            // paths: [path.resolve(lessDir)],
            compress: !this.$debug.noMinify,
        };
        this.$debug.data("Compiling:", compileOpts.filename);
        return utils.less
            .compileFile(Paths.DOCMA_LESS, compileOpts)
            .then((compiled) => {
                if (!compiled) {
                    return console.error(new Error("Not compiled less"));
                }
    ...
    compilePartials() {
        ...
        const partials = utils.glob.normalize(
        ...
        return utils.glob
            ...
            .then((files) => {
                ...
                return Promise.map(files, (filePath) => {
                    this.$debug.data(
                        "Compiling:",
                        path.relative(this.template.dirname, filePath),
                    );
            ...
            .then((results) => {
                ...
                const contentPartialPath = path.join(
                    this.template.templateDir,
                    "partials",
                    `${CONTENT_PARTIAL}.html`,
                );
                return fs.pathExists(contentPartialPath).then((exists) => {
                    if (!exists) {
                        this.$debug.warn(
                            "Missing Content Partial:",
                            contentPartialPath,
                        );
                        this.$debug.data("Creating:", contentPartialPath);
                        const compiled = dust.compile(
                            `<div id="${CONTENT_ELEM_ID}"></div>`,
                            CONTENT_PARTIAL,
                        );
    ...
    copyToDest() {
        ...
        ignoreList = ignoreList.concat([this.template.mainHTML, "partials/**"]);
        // normalize the globs (glob paths sep should be `/` even in windows.)
        ignoreList = ignoreList.map((item) =>
            utils.glob.normalize(templateDir, item),
        );

        return (
            utils.glob
                .match(`${templateDir}/**/*`, { ignore: ignoreList })
    ...
    writeServerConfig(routes) {
        ...
        if (this.appConfig.server === "apache") {
            ...
            this.$debug.info(
                "Generating Apache config file (.htaccess):",
                destHtaccess,
            );
            ...
            return fs.readFile(Paths.APACHE_CONF, "utf8").then((content) => {
        ...
        if (
            ["github", "static", "windows"].indexOf(this.appConfig.server) >= 0
        ) {
            ...
            return fs.readFile(Paths.REDIRECT_HTML, "utf8").then((html) => {
                return Promise.each(routes, (route) => {
                    ...
                    const reIndexFile = path.join(...p),
                        ...
                        reHtml = html
                            ...
                            .replace(
                                /%\{REDIRECT_PATH\}/g,
                                utils.path.removeLeadSlash(route.path),
                            ); // utils.path.ensureLeadSlash(route.path)
                    return fs.pathExists(reIndexFile).then((exists) => {
                        if (exists) return;
    ...
    writeMainHTML() {
        ...
        return (
            HtmlParser.fromFile(srcMainFile)
                .then((parser) => {
                    return parser.removeComments().edit((window, $) => {
                        const head = $("head");
                        ...
                        const metas = (this.appConfig.meta || []).concat([
                            docmaMeta,
                        ]);
                        ...
                        const title =
                            ...
                            "</title>" +
                            DOM.N_TAB; // eslint-disable-line
                        head.find("title")
                            .remove()
                            .end() // .remove('title') doesn't work
                            .prepend(title);

                        if (this.appConfig.favicon) {
                            const favicon = path.basename(
                        ...
                        DOM.insertAsFirst(window, "script", {
                            src: "js/docma-web.js",
                        });

                        // prepend docma.css before any javascript file
                        const docmaCss =
                            DOM.getStyleElem("css/docma.css") + DOM.N_TAB;
                        _insertBeforeFirst(
                            head,
                            "link[rel=stylesheet]",
                            docmaCss,
    ...
    _promiseTemplateProcess(fn, args = [], options = {}) {
        ...
        // ensure promise
        return Promise.resolve(this.template[fn](...args)).then((value) => {
            if (opts.resolveType === "undefined") return value;
            const gotType = utils.type(value);
            if (opts.resolveType !== gotType) {
                throw new Error(
                    `Template process '${fn}' resolved with an unexpected type of value. Expected '${opts.resolveType}', got '${gotType}'.`,
                );
            }
            return value;
        });
