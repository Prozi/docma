...
const info =
    ...
    "@ " +
    chalk.blue("https://onury.io/docma") +
    "\n" +
    chalk.yellow("Docma CLI ") +
    " @ " +
    chalk.blue("https://onury.io/docma/cli");
...
const argv = yargs
    ...
    .alias("h", "help")
    .version("v", "Output Docma version", pkg.version)
    .alias("v", "version")
    .option("c", {
        alias: "config",
        type: "string",
    ...
    .command(
        "serve [path]",
        "Start a static server for the generated SPA.",
        (yargs) => {
            yargs
                .options({
                    p: {
    ...
    .command(
        "template <cmd>",
        "Execute template related commands.",
        (yargs) => {
            yargs
                .command(
                    "init [path]",
    ...
    .showHelpOnFail(false, "Run with --help for available options.").argv;
...
if (cmds.indexOf("serve") >= 0) {
    ...
    Promise.resolve()
        .then(() => {
            return configFile ? utils.json.read(configFile) : {}; // use empty config
        })
        .then((config) => {
